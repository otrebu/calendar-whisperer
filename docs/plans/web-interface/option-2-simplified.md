# Option 2: Local Web Server (Simplified Always-Online)
...
- Clearer user expectations (web interface requires internet connection)

---

## Summary

Build a local web server that runs on the user's machine, launched via a CLI command like `calendar-whisperer web`. The web interface connects to this local server via HTTP API. Authentication tokens are shared between CLI and web through the server. This pattern is inspired by Prisma Studio and provides privacy-first, local-only operation with optional cloud deployment later.

**Key simplification:** Calendar data is fetched fresh from Microsoft Graph API on every request. No local caching or offline operation for calendar events.
...
│   │   │   ├── calendar.ts   # Calendar API (live fetching)
...
- Clearer error messages (network issues vs auth issues)

**API Flow:**

```
User requests events → Web API client → Local server → Microsoft Graph API
                                                              ↓
User sees events    ← Web UI        ← Local server    ← Live calendar data
```
...
export const webCommand = new Command("web")
  .description("Launch web interface (requires internet connection)")
...
- ✅ Live data fetching from Microsoft Graph
...
- [ ] Integration tests (server → Graph API)
...
✅ **Privacy-First:** All data stays on user's machine (passes through local server)
...
✅ **Simpler Architecture:** No cache invalidation or sync logic
✅ **Always Fresh Data:** Always displays latest calendar state
✅ **Faster Implementation:** 14 days vs 17 days (removed offline complexity)
✅ **Clearer Error Messages:** Network issues separate from auth issues

### Cons

❌ **Requires Internet:** Web interface non-functional without internet
❌ **No Offline Mode:** Cannot view cached events when offline
❌ **API Rate Limits:** More Graph API calls (no local cache buffer)
❌ **Latency:** Every view requires API roundtrip (500ms-2s)
...
- **Connectivity:** Reliable internet connection available
...
- **Simplicity:** Prefer simpler architecture over offline capability
...
1. **CLI Command**
   ...
   - ✅ Clear error messages for failures (port in use, network down)
   - ✅ Warns user if no internet connection detected
...
3. **Calendar API**
   ...
   - ✅ API proxies to Microsoft Graph (no local caching)
...
- ⚪ Optional caching layer (can be added later if needed)
...
- ⛔ Offline operation for calendar data
- ⛔ Local calendar data caching
...
### Calendar Router (Always-Online)

```typescript
// packages/server/src/routes/calendar.ts
import express from "express";
import { loadAuthResult, createGraphClient } from "@calendar-whisperer/core";

export function createCalendarRouter(cacheDirectory: string) {
  const router = express.Router();
  ...
  router.get("/events", async (req, res) => {
    try {
      ...
      // Fetch live data from Microsoft Graph
...
export function useCalendarEvents(date: Date, timeZone: string) {
  return useQuery({
    queryKey: ["calendar-events", date.toISOString(), timeZone],
    queryFn: () => getCalendarEvents(date, timeZone),
...
Option 2 (Simplified) provides a privacy-first, local-only solution that seamlessly shares authentication between CLI and web. By removing offline capabilities and calendar data caching, we've significantly simplified the implementation while maintaining the core benefits of local operation and token sharing.

**Trade-off:** Requires active internet connection for calendar data, but gains simpler architecture, faster implementation, and always-fresh data.

**Best for:** Privacy-conscious users with reliable internet, teams that want local-first tooling, gradual migration to web interface

**Not recommended if:** Need offline operation, have unreliable internet, want mobile access, prefer cloud deployment
...
**Risk Level:** Medium
**Recommended:** ⭐ Simplified approach for privacy-focused users with reliable connectivity
